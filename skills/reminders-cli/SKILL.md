...
5. Reminder text is passed as trailing positional arguments and may contain spaces; quote it. Notes go in `--notes`/`-n` and may start with `-` or span lines (e.g. a `- [ ] item` checklist). Reminder text that starts with `-` must come after `--`, placed after all options: `reminders add <list> --notes "- [ ] a" -- "- text"`.
